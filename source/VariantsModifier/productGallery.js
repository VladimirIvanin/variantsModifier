...
var updateProductGallery = function ($productGallery, data) {
  ...
  if (!self._updateGallery) {
    ...
    for (i; i < sliders.length; i++) {
      ...
      if (Gallery && Gallery[slider.method]) {
        var _index = $productGallery.find(slider.slideClass + ' [src*="'+self.activeImage+'"]').parents(slider.slideClass + ':first').index();
        self._slider = slider;
  ...
  if (self._updateGallery) {
    ...
    var $image = $productGallery.find(slider.slideClass + ' [src*="'+self.activeImage+'"]');
